...
export function getQueryUrlComponents(path: string): {
  queryOrigin: string;
  queryPath: string;
} {
...
export function isRepository<T>(
  repo: unknown | Repository<T>,
  ...
  try {
    if (Object.getPrototypeOf(repo).constructor.name === 'Repository')
      return true;
    return (
      typeof repo === 'object' && !('connection' in repo) && 'manager' in repo
    );
  } catch {
    return false;
  }
...
export function isFindOperator<T>(
  value: unknown | FindOperator<T>,
